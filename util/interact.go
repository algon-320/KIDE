...
func AskChoose(choice []string, message string) int {
	for {
		...
		var ans int
		fmt.Print(PrefixQuestion + message + " > ")
		fmt.Scan(&ans)
		if 1 <= ans && ans <= len(choice) {
			return ans - 1
		}

		fmt.Printf(PrefixCaution+"please choose the number from 1 to %d. try again ...\n", len(choice))
	}
}
...
func AskYesNo() bool {
	fmt.Print(PrefixQuestion + "Please respond with 'yes' or 'no' [y/N]: ")
	var resp string
	fmt.Scan(&resp)
